...
def draw_detect_dart(image, accumulator, feature):
    ...
    for (x, y, w, h) in darts:
        ...
        for x1,y1,r in circleCoordinates:       
            if( (x1 in range(x, x+w) and y1 in range(y, y+h)) and (feature[0] in range(x, x+w) and feature[1] in range(y, y+h)) ):
...
def feature_magnification(ori_img, img, value_threshold=220):
    temp_img = ori_img
    are_edges = img > value_threshold if True else img < value_threshold
    y_idxs, x_idxs = np.nonzero(are_edges)
    x_total = 0
    ...
    for i in range(len(x_idxs)):
        ...
        x_total += x
        y_total += y
        # edges_image = cv2.circle(temp_img, (x, y), 20, (255,0,0), 1)           
    x_average = x_total/len(x_idxs)
    y_average = y_total/len(x_idxs)
...
if __name__ == '__main__':
    if len(sys.argv)==2:
        ...
        for image_name in image_list:
            ...
            x_feature, y_feature = feature_magnification(image, img_sobel)

            #hough transform
            accumulator = hough_circle(image, img_sobel)

            #adaboost
            image_decteted, bounds_detected = draw_detect_dart(image_truth, accumulator,(int(x_feature), int(y_feature)))
            cv2.imwrite(result_folder + "detected.jpg", image_decteted)
